...
public class TransferService {
    ...
    // Transfer funds: debit source, add transfer record, credit beneficiary (if internal), etc.
    public String transfer(long userId, long fromAccountId, long beneficiaryId, BigDecimal amount) throws SQLException {
        if (amount == null || amount.compareTo(BigDecimal.ONE) < 0) return null;

        // Check ownership and sufficient balance
        if (!accountDao.isAccountOwnedByUser(fromAccountId, userId)) return null;
