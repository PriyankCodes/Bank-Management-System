...
<div class="container section-lg fade-in">
	...
	<!-- Filters -->
	<form method="get"
		action="${pageContext.request.contextPath}/admin/support-tickets"
		class="mb-4 row g-3 align-items-end">
		<div class="col-md-3">
			<label for="filterPriority" class="form-label">Filter by
				Priority:</label> <select id="filterPriority" name="priority"
				class="form-select">
				<option value="">All</option>
				<option value="LOW" ${param.priority == 'LOW' ? 'selected' : ''}>LOW</option>
				<option value="MEDIUM"
					${param.priority == 'MEDIUM' ? 'selected' : ''}>MEDIUM</option>
		...
		<table id="ticketsTable"
			...
			<tbody>
							...
							<form method="post"
								...
								<input type="hidden" name="ticketId" value="${ticket.id}" /> <input
									...
									<option value="IN_PROGRESS"
										${ticket.status == 'IN_PROGRESS' ? 'selected' : ''}>IN_PROGRESS</option>
						...
						<td>
							<form method="post"
								...
								<input type="hidden" name="ticketId" value="${ticket.id}" /> <input
									...
									<option value="HIGH"
										${ticket.priority == 'HIGH' ? 'selected' : ''}>HIGH</option>
								</select>
							</form>
						</td>
				...
				<c:if test="${empty tickets}">
					<tr>
						<td colspan="7" class="text-center text-muted">No support
							tickets found.</td>
					</tr>
				</c:if>
			</tbody>
